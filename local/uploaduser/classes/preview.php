...
require_once($CFG->dirroot.'/local/uploaduser/locallib.php');
...
class preview extends \html_table {
    ...
    public function __construct(\csv_import_reader $cir, array $filecolumns, int $previewrows) {
        ...
        $this->summary = get_string('uploaduserspreview', 'local_uploaduser');
        $this->head = array();
        $this->data = $this->read_data();

        $this->head[] = get_string('uucsvline', 'local_uploaduser');
